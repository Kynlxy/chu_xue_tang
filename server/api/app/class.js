...
var AboutClass = {
    ...
    getClassDetail(req, res) {
        var _classId = req.query.class_id,
            _sql = 'SELECT A.*, COUNT(A.class_id) AS total  FROM `class_detail` AS A INNER JOIN `class_user_relation` AS B  ON A.class_id = B.class_id WHERE A.class_id = ? GROUP BY A.id';
        client.query(_sql, _classId, (err, results) => {
            if (err) {
                return res.json({
                    code: 0,
